...
// app에게 사용자가 "/graphql" 일 때 미들웨어로 cors() 실행하고, bodyPaser 실행하고 그다음 graphqlHTTP를 실행할건데 graphql폴더 안에 있는 schema를 가지고 실행.
app.use(
  "/graphql",
  cors(),
  bodyParser.json(),
...
app.listen(app.get(`PORT`), () => {
  console.log(
    ` [REST SERVER START] :: ${process.env.PORT}, WITH GraphQL - MongoDB`
  );
});
